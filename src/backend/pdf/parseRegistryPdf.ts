...
function extractActiveMortgagesFromSummary(text: string): {
  ...
  // "3. (근)저당권 및 전세권 등 (을구)" - 내부 공백 변형 허용, 다음 섹션(4.)까지 또는 참고사항까지
  const sectionMatch = summaryText.match(
    /3\.\s*\(근\)저당권\s*및\s*전세권\s*등\s*\(\s*을구\s*\)([\s\S]*?)(?=\s*4\.\s*|\[\s*참\s*고\s*사\s*항\s*\]|출력일시|$)/
  );

  const section = sectionMatch?.[1] ?? "";

  console.log("[mortgage_summary] section_found:", sectionMatch !== null, "len:", section.length);
  ...
  if (/기록사항\s*없음/.test(section)) {
    console.log("[mortgage_summary] 기록사항 없음");
    return { foundSummary: true, mortgages: [] };
  }

  // 캡처 한도 120→200, 160→250 로 확장; 다음 근저당 행 시작도 종료 조건으로 추가
  const rowRegex =
    /(\d{1,3})\s+근저당권설정[\s\S]{0,200}?채권최고액\s*(?:금)?\s*([0-9][0-9,]{4,})\s*원[\s\S]{0,250}?근저당권자\s+(.+?)(?=\s+(?:\d{1,3}\s+근저당권설정|대상소유자|참\s*고\s*사\s*항|\[\s*참|4\.\s*|$))/g;
  ...
  for (const match of section.matchAll(rowRegex)) {
    ...
    console.log("[mortgage_summary] row:", { rank: originalRank, amount, creditor: creditor.slice(0, 20) });
  ...
  console.log("[mortgage_summary] total:", results.length);
  return {
    foundSummary: true,
...
function extractMortgagesFromBody(text: string): MortgageEntry[] {
  ...
  // 날짜 형식 두 가지 허용: "2024년1월15일" 또는 "2024. 1. 15." 또는 "2024.1.15"
  const DATE_PAT = `(?:\\d{4}년\\s*\\d{1,2}월\\s*\\d{1,2}일|\\d{4}\\.\\s*\\d{1,2}\\.\\s*\\d{1,2})`;
  const blockStartRegex = new RegExp(`(?:^|\\s)(\\d{1,3})\\s+근저당권설정\\s+${DATE_PAT}`, "g");
  const starts = Array.from(source.matchAll(blockStartRegex));

  console.log("[mortgage_body] 을구 블록 수:", starts.length);

  if (starts.length === 0) {
    // 블록 구조가 없으면 직접 스캔 (날짜 없이 패턴만으로)
    return extractMortgagesFromBodyDirect(source);
  }
  ...
  starts.forEach((startMatch, index) => {
    ...
    console.log("[mortgage_body] block:", { rank, amount, creditor: creditor.slice(0, 20) });
  ...
  console.log("[mortgage_body] total:", results.length);
  return results.sort((a, b) => a.rank - b.rank);
}

/** 블록 구조가 없을 때 을구 전체에서 직접 근저당 패턴을 스캔하는 3차 fallback */
function extractMortgagesFromBodyDirect(source: string): MortgageEntry[] {
  const results: MortgageEntry[] = [];
  const seen = new Set<string>();

  // 말소된 순위 먼저 수집
  const cancelledRanks = new Set<number>();
  for (const m of source.matchAll(/(\d{1,3})\s*번\s*근저당권설정[\s\S]{0,30}?말소|(\d{1,3})\s*번\s*근저당권[\s\S]{0,20}?해지/g)) {
    if (m[1]) cancelledRanks.add(Number(m[1]));
    if (m[2]) cancelledRanks.add(Number(m[2]));
  }

  // 패턴: 순위번호 + 근저당권설정 → 채권최고액 → 근저당권자 (한도 300자씩)
  const directRegex =
    /(\d{1,3})\s+근저당권설정[\s\S]{0,300}?채권최고액\s*(?:금)?\s*([0-9][0-9,]{4,})\s*원[\s\S]{0,300}?근저당권자\s+(.+?)(?=\s{2,}|\d{1,3}\s+근저당권설정|-- 이 하 여 백 --|$)/g;

  for (const match of source.matchAll(directRegex)) {
    const rank = Number(match[1]);
    if (cancelledRanks.has(rank)) continue;
    const amount = parseWonAmount(match[2]);
    const creditor = cleanSummaryCreditor(match[3]);
    console.log("[mortgage_body_direct] row:", { rank, amount, creditor: creditor.slice(0, 20) });
    if (!rank || !amount) continue;
    const key = `${rank}-${creditor}-${amount}`;
    if (seen.has(key)) continue;
    seen.add(key);
    results.push({ rank, creditor, amount });
  ...
  console.log("[mortgage_body_direct] total:", results.length);
  return results.sort((a, b) => a.rank - b.rank);
}
...
function extractMortgages(text: string): { mortgages: MortgageEntry[]; mortgageSummaryChecked: boolean } {
  const summaryResult = extractActiveMortgagesFromSummary(text);

  if (summaryResult.foundSummary) {
    return { mortgages: normalizeMortgageRanks(summaryResult.mortgages), mortgageSummaryChecked: true };
  }

  return { mortgages: normalizeMortgageRanks(extractMortgagesFromBody(text)), mortgageSummaryChecked: false };
}
...
export function parseRegistryText(params: {
  ...
  const { mortgages, mortgageSummaryChecked } = extractMortgages(compactText);
