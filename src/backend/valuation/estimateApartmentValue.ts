...
function estimatePriorityRepaymentAmount(params: {
  ...
  if (sido.includes("서울")) return Math.min(deposit, 55_000_000);
  if (sido.includes("경기") || sido.includes("인천")) return Math.min(deposit, 48_000_000);
  if (
    sido.includes("부산") || sido.includes("대구") ||
    sido.includes("광주") || sido.includes("대전") ||
    sido.includes("울산") || sido.includes("세종")
  ) return Math.min(deposit, 28_000_000);
...
export async function estimateApartmentValue(input: ValuationInput): Promise<ValuationResult> {
  ...
  // PDF 자동추출 근저당 채권최고액 합산 (seniorMortgageAmount = 0 하드코딩 제거)
  const mortgages: MortgageItem[] = (input.rightsRisk?.mortgages ?? []).map((m) => ({
    rank: m.rank,
    creditor: m.creditor,
    amount: m.amount ?? 0,
  }));
  const seniorMortgageAmount = mortgages.reduce((sum, m) => sum + m.amount, 0);
  ...
  const riskAdjustedPrice = weightedAveragePrice !== undefined
  ...
  const sidoLabel = region.sido?.includes("서울") ? "서울 기준 (5,500만원)" :
    (region.sido?.includes("경기") || region.sido?.includes("인천")) ? "수도권 기준 (4,800만원)" :
    "지방 기준 적용";
