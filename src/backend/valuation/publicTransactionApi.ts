...
function getFloorSimilarityScore(params: {
  ...
  if (floorDifference <= 2) {
    return { score: 14, reason: "유사층(±2층)" };
  }

  if (floorDifference <= 5) {
    return { score: 8, reason: "인접층(±5층)" };
  }

  if (floorDifference <= 10) {
    return { score: 3, reason: `층수 차이 ${floorDifference}층` };
  }

  return { score: -8, reason: `층수 차이 큼(${floorDifference}층)` };
}
...
async function fetchApartmentTradeApi(
  ...
  try {
    ...
    for (const match of itemMatches) {
      ...
      const area = Number(itemXml.match(/<excluUseAr>(.*?)<\/excluUseAr>/)?.[1] ?? "");
      const dealAmount = Number(
        (itemXml.match(/<dealAmount>(.*?)<\/dealAmount>/)?.[1] ?? "")
          .replace(/,/g, "")
          .trim()
      );
      const dealYear = Number(itemXml.match(/<dealYear>(.*?)<\/dealYear>/)?.[1] ?? 0);
      const dealMonth = Number(itemXml.match(/<dealMonth>(.*?)<\/dealMonth>/)?.[1] ?? 0);
      const dealDay = Number(itemXml.match(/<dealDay>(.*?)<\/dealDay>/)?.[1] ?? 0);
      const floor = Number(itemXml.match(/<floor>(.*?)<\/floor>/)?.[1] ?? 0);
      const buildYear = Number(itemXml.match(/<buildYear>(.*?)<\/buildYear>/)?.[1] ?? 0);
      const dealType =
      ...
      if (areaDifferenceM2 !== undefined && areaDifferenceM2 > areaToleranceM2) {
        continue;
      }
      ...
      const transactionKey = [dealAmount, dealYear, dealMonth, dealDay, area, floor, aptNm].join("|");
  ...
  for (const rule of FALLBACK_RULES) {
    ...
    for (const dealYearMonth of recentMonths) {
      ...
      for (const tx of monthlyTransactions) {
        const key = [tx.dealAmount, tx.dealYear, tx.dealMonth, tx.dealDay, tx.area, tx.floor].join("|");
        if (seenGlobalKeys.has(key)) continue;
        seenGlobalKeys.add(key);
        apiTransactions.push(tx);
      }

      const sameApartmentCount = apiTransactions.filter((tx) => tx.isSameApartment).length;

      if (sameApartmentCount >= rule.minimumComparableCount || apiTransactions.length >= 5) {
        break;
      }
    }
