...
function extractBuildYear(value?: string): number | undefined {
  ...
  if (!year || year < 1900 || year > new Date().getFullYear() + 1) return undefined;
  return year;
}
...
function calculateNameSimilarity(target?: string, candidate?: string) {
  ...
  if (targetName.includes(candidateName) || candidateName.includes(targetName)) return 85;
  let score = 0;
  for (const char of targetName) {
    if (candidateName.includes(char)) score += 1;
  }
...
async function requestPublicDataItem<T>(
  ...
  try {
    ...
    if (!url) { console.warn("PUBLIC_DATA_API_KEY is missing."); return undefined; }
    const response = await fetch(url.toString(), { method: "GET", cache: "no-store" });
    if (!response.ok) {
      const body = await response.text();
      console.warn("apartment_item_api_failed", { status: response.status, url: url.toString(), body: body.slice(0, 500) });
      return undefined;
    }
...
// ✅ BUG 1 수정: getAphusBassInfo → getAphusBassInfoV4
...
// ✅ BUG 2 수정: BassInfo URL → DtlInfoV4 로 변경
export async function fetchApartmentDetailInfo(
  ...
  if (!item) return undefined;
  const subwayWalkingTimeText = String(item.kaptdWtimesub ?? "") || undefined;
  return {
    kaptCode: String(item.kaptCode ?? "") || undefined,
    kaptName: String(item.kaptName ?? "") || undefined,
...
// ✅ BUG 3 수정: AptBasisInfoServiceV4/getLegaldongAptList → AptListService2/getLegaldongAptList3
export async function fetchLegalDongApartmentList(params: {
  ...
  try {
    ...
    const url = createPublicDataUrl(
      "https://apis.data.go.kr/1613000/AptListService2/getLegaldongAptList3",
      { bjdCode: params.legalDongCode },
      { json: false }
    );
    if (!url) { console.warn("PUBLIC_DATA_API_KEY is missing."); return []; }
    const response = await fetch(url.toString(), { method: "GET", cache: "no-store" });
    const xml = await response.text();
    if (!response.ok) {
      console.warn("apartment_list_api_failed", { status: response.status, url: url.toString(), body: xml.slice(0, 500) });
      return [];
    }
    console.log("apartment_legal_dong_raw", { status: response.status, legalDongCode: params.legalDongCode, sample: xml.slice(0, 300) });
    const itemMatches = [...xml.matchAll(/<item>([\s\S]*?)<\/item>/g)];
    return itemMatches.map((match) => {
      const itemXml = match[1];
      return {
        kaptCode: itemXml.match(/<kaptCode>(.*?)<\/kaptCode>/)?.[1]?.trim() || undefined,
        kaptName: itemXml.match(/<kaptName>(.*?)<\/kaptName>/)?.[1]?.trim() || undefined,
        kaptAddr: itemXml.match(/<kaptAddr>(.*?)<\/kaptAddr>/)?.[1]?.trim() || undefined,
        doroJuso: itemXml.match(/<doroJuso>(.*?)<\/doroJuso>/)?.[1]?.trim() || undefined,
        bjdCode:  itemXml.match(/<bjdCode>(.*?)<\/bjdCode>/)?.[1]?.trim()  || undefined
      };
    });
  } catch (error) {
...
export async function findApartmentKaptCode(params: {
  ...
  const candidates = await fetchLegalDongApartmentList({ legalDongCode: params.legalDongCode });
  console.log("kapt_target_lookup_input", { legalDongCode: params.legalDongCode, buildingName: params.buildingName, candidateCount: candidates.length });
  if (!candidates.length) return undefined;
  const scored = candidates
    .map((candidate) => ({ candidate, score: calculateNameSimilarity(params.buildingName, candidate.kaptName) }))
    .sort((a, b) => b.score - a.score);
  console.log("kapt_target_lookup_candidates", scored.slice(0, 5).map((item) => ({
    score: item.score, kaptCode: item.candidate.kaptCode, kaptName: item.candidate.kaptName,
    kaptAddr: item.candidate.kaptAddr, bjdCode: item.candidate.bjdCode
  })));
  const best = scored[0];
...
export async function fetchApartmentMetaInfoByLegalDong(params: {
  ...
  const kaptCode = await findApartmentKaptCode({ legalDongCode: params.legalDongCode, buildingName: params.buildingName });
  if (!kaptCode) return undefined;
...
function calculateApartmentSimilarity(params: {
  ...
  let score = calculateNameSimilarity(params.targetName, params.candidateName);
  if (params.targetBuildYear && params.candidateBuildYear) {
    const diff = Math.abs(params.targetBuildYear - params.candidateBuildYear);
    if (diff <= 1) score += 15;
    else if (diff <= 3) score += 8;
    else if (diff >= 10) score -= 10;
  }
  if (params.targetHouseholdCount && params.candidateHouseholdCount) {
    const ratio = Math.abs(params.targetHouseholdCount - params.candidateHouseholdCount) / params.targetHouseholdCount;
...
export async function findApartmentKaptCodeInLegalDong(params: {
  ...
  const candidates = await fetchLegalDongApartmentList({ legalDongCode: params.legalDongCode });
  console.log("kapt_transaction_lookup_input", { legalDongCode: params.legalDongCode, apartmentName: params.apartmentName, candidateCount: candidates.length });
  if (!candidates.length) return undefined;
  const scored = candidates
    .map((candidate) => ({ candidate, score: calculateNameSimilarity(params.apartmentName, candidate.kaptName) }))
    .sort((a, b) => b.score - a.score);
  console.log("kapt_transaction_lookup_candidates", scored.slice(0, 5).map((item) => ({
    score: item.score, kaptCode: item.candidate.kaptCode, kaptName: item.candidate.kaptName,
    kaptAddr: item.candidate.kaptAddr, bjdCode: item.candidate.bjdCode
  })));
  const best = scored[0];
