...
// ─── VWorld 주소검색으로 법정동코드 조회 ────────────────────────────────────
async function fetchLegalDongCodeFromVWorld(
  address: string
  ...
  try {
    ...
    for (const type of ["road", "parcel"] as const) {
      ...
      if (data?.response?.status !== "OK" || !result) continue;

      // VWorld 정제 주소에서 법정동코드 추출
      // refined.addrid 형태: "법정동코드(10자리) 지번"
      const addrid: string = result.refined?.addrid ?? "";
      const codeMatch = addrid.match(/^(\d{10})/);
      if (codeMatch) {
        console.log("vworld_legal_dong_code_found", {
          address,
          code: codeMatch[1],
          type,
        });
      ...
      // addrid 없으면 refined.text로 재시도 매핑
      const refinedText: string = result.refined?.text ?? "";
      if (refinedText) {
        const mapped = lookupFromText(refinedText);
    ...
    console.warn("vworld_geocode_failed", e);
    return undefined;
  }
}

// ─── VWorld 키워드 검색으로 법정동코드 조회 (폴백) ──────────────────────────
async function fetchLegalDongCodeFromVWorldSearch(
  address: string
): Promise<string | undefined> {
  try {
    const apiKey = process.env.VWORLD_API_KEY;
    ...
    url.searchParams.set("category", "L4"); // 법정동
    url.searchParams.set("format", "json");
    ...
    const codeMatch = id.match(/(\d{10})/);
    if (codeMatch) {
      console.log("vworld_search_legal_dong_code_found", {
        address,
        code: codeMatch[1],
      });
      return codeMatch[1];
    ...
    console.warn("vworld_search_failed", e);
    return undefined;
  }
}
...
function lookupFromText(text: string): string | undefined {
  // 가장 긴 키부터 매칭 (구체적인 것 우선)
  const sorted = Object.keys(LEGAL_DONG_CODES).sort(
...
// ─── public: 하드코딩 → VWorld 주소검색 → VWorld 키워드검색 순으로 시도 ────
export async function findLegalDongCode(
  ...
  // 2. VWorld 주소검색 API (도로명/지번)
  const vworldCode = await fetchLegalDongCodeFromVWorld(key);
  if (vworldCode) return vworldCode;

  // 3. VWorld 키워드 검색 폴백
  const vworldSearchCode = await fetchLegalDongCodeFromVWorldSearch(key);
  if (vworldSearchCode) return vworldSearchCode;
