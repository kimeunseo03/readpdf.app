...
function DisplayRow({ label, value }: { label: string; value?: string }) {
  return (
    <div className="grid grid-cols-[92px_1fr] gap-3 border-b border-slate-100 py-3 text-sm last:border-b-0">
...
export function ParseResultView({ response }: { response: ParseApiResponse }) {
  ...
  const { rightsRisk, review } = parseResult;
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
            ...
            {isEditing ? (
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <EditInput
                  label="시도"
                  value={editableProperty.sido}
                  onChange={(value) => updateField("sido", value)}
                />
                <EditInput
                  label="시군구"
                  value={editableProperty.sigungu}
                  onChange={(value) => updateField("sigungu", value)}
                />
                <EditInput
                  label="읍면동"
                  value={editableProperty.eupmyeondong}
                  onChange={(value) => updateField("eupmyeondong", value)}
                />
                <EditInput
                  label="건물명"
                  value={editableProperty.buildingName}
                  onChange={(value) => updateField("buildingName", value)}
                />
                <EditInput
                  label="동"
                  value={editableProperty.buildingDong}
                  onChange={(value) => updateField("buildingDong", value)}
                />
                <EditInput
                  label="호수"
                  value={editableProperty.unitNumber}
                  onChange={(value) => updateField("unitNumber", value)}
                />
                <EditInput
                  label="층"
                  value={editableProperty.floor}
                  onChange={(value) => updateField("floor", value)}
                />
                <EditInput
                  label="전유면적㎡"
                  value={editableProperty.exclusiveAreaM2}
                  onChange={(value) => updateField("exclusiveAreaM2", value)}
                />
                <EditInput
                  label="대지권"
                  value={editableProperty.landRightRatio}
                  onChange={(value) => updateField("landRightRatio", value)}
                />
              </div>
            ) : (
              <dl>
